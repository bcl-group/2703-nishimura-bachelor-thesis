...
plt.title('Number of Altruistic Actions per Step', fontsize=15)
plt.xlabel('Step', fontsize=15)
plt.ylabel('Altruistic Actions Count', fontsize=15)
plt.tick_params(labelsize=15)
plt.grid(True)
plt.savefig(OUTPUT_DIR /'altruism_history.png')  
plt.close()

# 2. 個体数推移のグラフ描画と保存
plt.figure(figsize=(10, 6))
plt.plot(history_steps, history_population, marker='', linestyle='-', color='g')
plt.title('Population Size over Time', fontsize=15)
plt.xlabel('Step', fontsize=15)
plt.ylabel('Number of Agents', fontsize=15)
plt.tick_params(labelsize=15)
plt.grid(True)
plt.savefig(OUTPUT_DIR /'population_history.png')  
plt.close()
