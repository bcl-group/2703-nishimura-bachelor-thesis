...
FOOD_DURABILITY = 5      # 餌の耐久値

# 報酬・閾値パラメータ
ENERGY_LOSS_PER_STEP = 0.1 # 1ステップの消費エネルギー
REWARD_FOOD = 1.0          # 餌獲得時の報酬
CLONE_THRESHOLD = 100.0    # 複製閾値
DEATH_THRESHOLD = -10.0      # 死亡閾値
TRANSFER_AMOUNT = 5.0      # 利他行動1回あたりのエネルギー譲渡量
STARVING_THRESHOLD = DEATH_THRESHOLD + 20.0  # 他者の「低エネルギー」を判定する閾値

class Agent:
    def __init__(self, x, y, weights=None):
        self.x = x
        self.y = y
        self.energy = DEATH_THRESHOLD + 20.0
        self.age = 0
        if weights is None:
    ...
    def step(self):
        ...
        for agent in self.agents:
            ...
            if agent.energy >= CLONE_THRESHOLD:
                agent.energy = DEATH_THRESHOLD + 20.0 
...
plt.title('Number of Altruistic Actions per 1 Steps')
plt.xlabel('Step')
plt.ylabel('Altruistic Actions Count')
plt.grid(True)
